...
async function delete_container(user_id) {
    let response = await CTFd.fetch("/api/v1/plugins/ctfd-kubernetes/admin/container?user_id=" + user_id, {
        method: "DELETE",
...
async function renew_container(user_id) {
    let response = await CTFd.fetch(
        "/api/v1/plugins/ctfd-kubernetes/admin/container?user_id=" + user_id, {
        method: "PATCH",
        credentials: "same-origin",
        headers: {
            Accept: "application/json",
