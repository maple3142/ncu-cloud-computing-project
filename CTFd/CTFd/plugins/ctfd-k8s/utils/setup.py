...
from ..models import KubernetesRedirectTemplate, db
...
def setup_default_configs():
    ...
    db.session.add(KubernetesRedirectTemplate(
        ...
        '{{ get_config("kubernetes:frp_http_domain_suffix", "") }}'
        '{% if get_config("kubernetes:frp_http_port", "80") != 80 %}:{{ get_config("kubernetes:frp_http_port") }}{% endif %}/',
        '''
...
'''
    ...
    db.session.add(KubernetesRedirectTemplate(
        'direct',
        'nc {{ get_config("kubernetes:frp_direct_ip_address", "127.0.0.1") }} {{ container.port }}',
        '''
