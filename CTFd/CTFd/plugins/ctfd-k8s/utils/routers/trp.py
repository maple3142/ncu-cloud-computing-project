...
from ..db import DBContainer, KubernetesContainer
...
class TrpRouter(BaseRouter):
    ...
    def __init__(self):
        super().__init__()
        self.ses = session()
        self.url = get_config('kubernetes:trp_api_url', '').rstrip("/")
        self.common = ''
        for container in DBContainer.get_all_alive_container():
            self.register(container)

    @staticmethod
    def get_domain(container: KubernetesContainer):
        domain = get_config('kubernetes:trp_domain_suffix', '127.0.0.1.nip.io').lstrip('.')
    ...
    def access(self, container: KubernetesContainer):
        ...
        port = get_config('kubernetes:trp_listening_port', 1443)
        if ch_type == 'direct':
