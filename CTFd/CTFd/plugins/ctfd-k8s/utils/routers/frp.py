...
class FrpRouter(BaseRouter):
    ...
    def __init__(self):
        super().__init__()
        self.ses = session()
        self.url = get_config("kubernetes:frp_api_url").rstrip("/")
        self.common = ''
        try:
    ...
    def reload(self, exclude=None):
        ...
        try:
            if not self.common:
                common = get_config("kubernetes:frp_config_template", '')
                if '[common]' in common:
                    self.common = common
                else:
                    remote = self.ses.get(f'{self.url}/api/config')
                    assert remote.status_code == 200
                    set_config("kubernetes:frp_config_template", remote.text)
    ...
    def access(self, container: KubernetesContainer):
        if container.challenge.redirect_type == 'direct':
            return f'nc {get_config("kubernetes:frp_direct_ip_address", "127.0.0.1")} {container.port}'
        elif container.challenge.redirect_type == 'http':
            host = get_config("kubernetes:frp_http_domain_suffix", "")
            port = get_config("kubernetes:frp_http_port", "80")
    ...
    def unregister(self, container: KubernetesContainer):
        if container.challenge.redirect_type == 'direct':
            try:
                ...
                logging.log(
                    'kubernetes', 'Error deleting port from cache',
